...
export default observer(class CategorySelectPopup extends React.Component {
  ...
  updatePageData = async () => {
    ...
    // };
    let response = await pagingCategories({ pageIndex: this.state.page, pageSize: this.state.rowsPerPage });
    ...
    if (selectedArr && selectedArr.length > 0) {
      for (let i = 0; i < arr.length; i++) {
        for (let j = 0; j < selectedArr.length; j++) {
          if (arr[i].id === selectedArr[j].category.id) {
  ...
  componentWillMount() {
    ...
    this.setState({ selectedItems: selectedCategories ? selectedCategories : [] });
  }
  ...
  onSelectRows = rowData => {
    ...
    if (rslts[index]?.tableData.checked) {
      arr.push({ category: rslts[index] });
    } else {
      arr = arr.filter((e) => e?.category.id !== rslts[index]?.id);
    }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        >
          <span className="mb-20 text-white">Chọn danh mục</span>
          <IconButton

...
import DeliveryPlaceList from "./DeliveryPlaceList";
import DeliveryPlaceForm from "./DeliveryPlaceForm";

const DeliveryPlaceIndex = () => {
  ...
  useEffect(() => {
    resetDelPlaceStore();
    updatePageData()
  }, [resetDelPlaceStore, updatePageData])

  return (
    <section className="cards-container">
      <h4 className="cards-header">{t("deliveryPlace.title")}</h4>
        ...
        <Grid className="mb-16" container spacing={3}>
          <Grid item md={6} xs={4}>
            <Button
              ...
              startIcon={<AddIcon />}
              onClick={() => handleEditDelPlace()}
            >
              {!isMobile && t("general.button.add")}
            </Button>
            {selectedDelPlaceList.length > 0 && (
              <Button
      ...
      <DeliveryPlaceForm open={shouldOpenEditorDialog} />
...
export default memo(observer(DeliveryPlaceIndex));